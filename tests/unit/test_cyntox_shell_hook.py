from scripts import cyntox_shell_hook
...
def test_shell_hook_denies_public_url_when_internet_is_off() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(shell_payload("curl https://example.com"))

    assert decision(response) == "deny"


def test_shell_hook_denies_network_plus_secret_like_data() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(
...
def test_shell_hook_asks_for_local_shell_command() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(shell_payload("pytest tests/unit"))

    assert decision(response) == "ask"


def test_shell_hook_denies_raw_git_diff_terminal_flood() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(shell_payload("git diff"))
...
def test_shell_hook_allows_bounded_git_diff() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(shell_payload("git diff --stat"))

    assert decision(response) == "ask"


def test_shell_hook_allows_bounded_git_log() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(shell_payload("git log --oneline -n30"))
...
def test_shell_hook_denies_raw_log_dump_without_bound() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(shell_payload("Get-Content -Raw app.log"))
...
def test_shell_hook_allows_log_tail() -> None:
    response = cyntox_shell_hook.evaluate_pre_tool_use(
        shell_payload("Get-Content -Tail 50 app.log")
    )

...
The included fixture is the complete verified target. Real OS integration is through a target plugin/manifest that must describe source root, immutable base commit, build profiles, boot method, QEMU settings, readiness patterns, test transport, symbols, instrumentation, and cleanup. Manifest-backed real-target builds validate the manifest, create a detached disposable Git worktree at `source.base_commit`, run only the declared argv-vector build commands with the declared environment allowlist, and hash the declared build artifacts. Gate L remains blocked until an authorized real OS source path and existing build entry point are provided.

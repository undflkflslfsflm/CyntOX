...
| 15. Acceptance | Complete except Gate L | Main checkout and clean checkout live selftests passed at verified code commit `207d47c`; artifact index, `PROOF.json`, `FINAL_REPORT.md`, a machine-readable Gate L blocker report, a gate-by-gate requirements trace, and runnable acceptance audit are produced. The audit now verifies required artifact contents, key evidence CAS artifacts, the Gate L blocker report, the requirements trace, referenced selftest proof artifacts, the clean checkout's exact source commit, live model/Qwen Code stdout, selftest dependency reproducibility checks, and the recorded acceptance-audit artifact, and rejects stale, hand-edited, or inconsistent evidence blobs. `selftest` also runs `oslab target blocker-report --json` and `oslab acceptance trace --json` so both report generators are part of the proof sequence. The CLI now accepts the spec-style top-level `campaign` and `eval` workflows, and repo-local `oslab.ps1`/`oslab.cmd` launchers provide a one-line entry point. Overall goal remains blocked until an authorized real OS source path and build entry point are supplied. |

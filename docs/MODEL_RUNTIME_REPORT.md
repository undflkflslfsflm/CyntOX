...
- Worker model: `huihui-qwen3.8-27b-abliterated:latest`
- Qwen Code wrapper model: `qwen-os-lab-worker:latest`
- Runtime: Ollama 0.33.2
- Architecture reported by Ollama: `qwen35`
...
- Validated lab context: 16,384 tokens for Qwen Code MCP smoke; 8,192 tokens for the direct worker config
...
- Tool-mediated reliability: passed the constrained Qwen Code MCP smoke with only `mcp__oslab__policy_remaining_budget` and `mcp__oslab__fixture_explain` visible
...
| vLLM | Not installed | Upstream lists Qwen3 causal models, but current GGUF support is documented as experimental and moved to an out-of-tree plugin. Not selected for the current GGUF worker. |
| SGLang | Not installed | Upstream has Qwen3/Qwen3.8 recipes and OpenAI-compatible serving, but no local SGLang server or HF-format model is installed here. |
...
| AirLLM | Python package not installed | Upstream releases mention Qwen3.8-27B support with very low VRAM through streaming, but this is a disk/offload path and not a speed path. It is not used because the resident Ollama model is already fast and no AirLLM split/cache exists locally. |
...
- `fast`: resident Ollama Qwen, short/medium context, default for normal worker calls, QEMU triage, and patch loops.
...
- SGLang Qwen3 documentation: https://docs.sglang.io/cookbook/autoregressive/Qwen/Qwen3
